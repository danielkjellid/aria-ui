...
import AriaSelect from './AriaSelect.vue'
...
const Template: Story<SelectProps> = (args: SelectProps) => ({
  components: { AriaSelect },
  setup() {
    return { args }
  },
  template: '<aria-select v-bind="args"><option v-for="i in 31" :key="i" :value="i" :selected="I == 1">{{i}}</option></aria-select>'
})

const TemplateWithIconSlot: Story<SelectProps> = (args: SelectProps) => ({
  components: { AriaSelect, PrinterIcon },
  setup() {
    return { args }
  },
  template: '<aria-select v-bind="args"><template #icon><PrinterIcon class="h-5 w-5 text-gray-400"/></template><option v-for="i in 31" :key="i" :value="i" :selected="I == 1">{{i}}</option></aria-select>'
})
